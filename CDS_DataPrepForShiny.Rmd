...
dta <- read_excel("~/Dropbox/0Kids/CDS/CommonDataSet.xlsx", 
                  range = "A3:CY1000",
                  col_names = TRUE)%>%
    select(-starts_with("..."))%>%
...
dta<-dta%>%
    mutate(
        ...
        # STUDENT POPULATION  
        ratio_underft_mf = round(num_under_ftmale / num_under_ftfemale, 2),   
        pc_total_under = num_under / num_under + num_graduate, 
...
dta<-dta%>%
    ...
    mutate(
        ...
        temp = mean(numtransenroll_total)/mean(num_under), 
        relnumtransapp_total = replace(relnumtransapp_total, temp<0.05, NA), 
        relnumtransadmit_total = replace(relnumtransadmit_total, temp<0.05, NA), 
        relnumtransenroll_total = replace(relnumtransenroll_total, temp<0.05, NA),
...
dta <- dta%>%
    ...
    mutate(
        ...
        group=paste(groupall, groupsize, groupwomen, groupopen, grouppublic) 
    )
```
